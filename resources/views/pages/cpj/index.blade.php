            ...
            <div class="card">
              <div class="card-header d-flex justify-content-between flex-row">
                ...
                <div class="right-container">
                  <a class="btn btn-outline-primary btn-rounded" href="{{ route('cpj.create') }}">Create</a>
                </div>
              </div>
              <div class="card-body">
                @if (count($errors) > 0)
                          ...
                          <td class="text-right">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="aksiDropdown">
                                    <a href="{{route('cpj.edit', $key->id)}}" class="dropdown-item"><i class="ik ik-edit-2"> </i>Edit</a>
                                    <form method="get" action="{{ route('cpj.show', $key->id) }}">
                                      @csrf
                                      @method('GET')
                                      <button class="dropdown-item" type="submit"><i class="ik ik-eye"></i> Journal</button>
                                    </form>
                                    <a href="{{route('cpj.retur', $key->id)}}" class="dropdown-item"><i class="ik ik-rotate-ccw"> </i>Retur Pembelian</a>
                                    <form method="post" action="{{ route('cpj.destroy', $key->id) }}">
                                      @csrf
                                      @method('DELETE')
