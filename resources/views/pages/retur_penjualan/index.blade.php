...
@section('title', 'AccountMin - Retur Penjualan')
                          ...
                          <td class="text-right">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="aksiDropdown">
                                    <a href="{{route('retur_penjualan.edit', $key->id)}}" class="dropdown-item"><i class="ik ik-edit-2"> </i>Edit</a>
                                    <form method="get" action="{{ route('retur_penjualan.show', $key->id) }}">
                                      @csrf
                                      @method('GET')
                                      <button class="dropdown-item" type="submit"><i class="ik ik-eye"></i> Journal</button>
                                    </form>
                                    <form method="post" action="{{ route('retur_penjualan.destroy', $key->id) }}">
                                      @csrf
                                      @method('DELETE')
