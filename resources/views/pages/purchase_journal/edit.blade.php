...
@section('title', 'AccountMin - Edit Purchase Journal')
    ...
    <div class="row" id="app">
      <div class="col-md-12">
        @if (count($errors) > 0)
          ...
          @endif
        <form class="forms-sample" action="{{route('purchase_journal.update', $cashbanks->id)}}" method="post">
          @csrf
          @method('PUT')
          <div class="card">
  ...
  computed: {
    ...
    items() {
      ...
      @foreach($items as $key)
        @if($inventories->where('items_id',$key->id)->where('purchasejournal_id', $cashbanks->id)->sum('total') < $Item_count)
